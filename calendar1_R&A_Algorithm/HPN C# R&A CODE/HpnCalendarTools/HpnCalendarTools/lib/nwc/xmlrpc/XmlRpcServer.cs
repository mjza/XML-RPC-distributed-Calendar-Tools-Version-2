...
public class XmlRpcServer : IEnumerable
  {
    ...
    private IDictionary _handlers; //keep a list of all handlers and their objects in the server, these objects will be called by other clients
    private XmlRpcSystemObject _system;
    private WaitCallback waitCallback;
    private Thread thread;//Mjzedit added
    ...
    public XmlRpcServer(IPAddress address, int port)
      {
	    ...
	    waitCallback = new WaitCallback(WaitCallback);
	        ...
	        lock (this)
	          {
                  ...
                  {
                      if (_myListener == null)
                          _myListener = new TcpListener(_address, _port);
                      _myListener.Start();
                  }
	    ...
	    try
	      {
	        ...
	        {
              Logger.WriteEntry("An Exception Occurred while stopping :" + e.ToString(), LogLevel.Error);
	        }
      }
    ...
    static public void HttpHeader(string sHttpVersion, string sMIMEHeader, long iTotBytes, string sStatusCode, TextWriter output)
      {
        ...
        output.WriteLine(sHttpVersion + sStatusCode);
	    //sBuffer +=  "Connection: close\r\n";
        output.WriteLine("Connection: close");
        if (iTotBytes > 0)
        {
            //sBuffer += "Content-Length: " + iTotBytes + "\r\n";
            output.WriteLine("Content-Length: " + iTotBytes);
        }
	    //sBuffer += "Server: XmlRpcServer \r\n";
        output.WriteLine("Server: XmlRpcServer ");
	    //sBuffer += "Content-Type: " + sMIMEHeader + "\r\n";
        output.WriteLine("Content-Type: " + sMIMEHeader);
	    //sBuffer += "\r\n";
        output.WriteLine("");
